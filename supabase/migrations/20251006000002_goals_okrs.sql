...
create policy "goals_owner_rw" on public.goals for all to authenticated
using (owner = auth.uid())
with check (owner = auth.uid());

create policy "goals_project_read" on public.goals for select to authenticated
using (
  project_id is null
  or project_id in (
...
create policy "krs_owner_rw" on public.key_results for all to authenticated
...
create policy "krs_project_read" on public.key_results for select to authenticated
using (goal_id in (
...
create policy "goal_updates_owner_rw" on public.goal_updates for all to authenticated
using (goal_id in (select id from public.goals where owner = auth.uid()))
with check (goal_id in (select id from public.goals where owner = auth.uid()));
