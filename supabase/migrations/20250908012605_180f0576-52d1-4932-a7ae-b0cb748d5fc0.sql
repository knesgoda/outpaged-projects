...
-- Drop existing permissive policies that may expose customer data
...
ALTER VIEW public.task_assignees_with_profiles SET (security_invoker = true);
...
-- 8. Create trigger for customer data access logging
...
DROP TRIGGER IF EXISTS audit_customer_data_access_trigger ON public.tickets;
CREATE TRIGGER audit_customer_data_access_trigger
