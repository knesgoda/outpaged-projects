-- CRITICAL: Fix remaining publicly exposed sensitive tables (Fixed Version)

-- 1. CRITICAL: Secure tickets table - drop existing policies first
...
-- 2. CRITICAL: Secure user_sessions table - drop existing policies first
DROP POLICY IF EXISTS "Users can view their own sessions only" ON public.user_sessions;
DROP POLICY IF EXISTS "Users can insert their own sessions only" ON public.user_sessions;
DROP POLICY IF EXISTS "Users can update their own sessions only" ON public.user_sessions;
DROP POLICY IF EXISTS "Users can delete their own sessions only" ON public.user_sessions;
...
-- 3. CRITICAL: Secure password_history table - drop existing policies first
